...
class MeObject(dict):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(obj, dict) and '_type' in obj and obj['_type'] == 'pointer':
            if self.overLoadGet:
                if '_content' in obj:
                    return obj['_content']
                else:
    ...
    def __setitem__(self, key, val):
        ...
        if self.objectId != None and '$set' not in self.dirty:
            self.dirty['$set'] = {}
        ...
        if isinstance(val, MeObject):
            ...
            if self.objectId != None:
                ...
                self.dirty[key] = obj
        # 不推荐走这个分支!!!
        elif isinstance(val, dict) and '_type' in val and val['_type'] == 'pointer':
    ...
    def append(self, key, val):
        if self.objectId != None and '$push' not in self.dirty:
            self.dirty['$push'] = {}
    ...
    def fetch(self, k):
        if isinstance(self[k], dict) and '_type' in self[k] and self[k]['_type'] == 'pointer':
            db = self.appDb
            if '_db' in self[k]:
                db = self[k]['_db']
    ...
    def copySelf(self, obj, child=False):
        ...
        for k in obj:
            # 如果是引用类型，取出引用内容
            if isinstance(obj[k], dict) and '_type' in obj[k] and obj[k]['_type'] == 'pointer':
                db = self.appDb
                if '_db' in obj[k]:
                    db = obj[k]['_db']
