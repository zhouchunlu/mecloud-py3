...
class Db:
    ...
    #             # 返回一个事务对象
    #             if Db.transaction_map.has_key(action_name):
    #                 return copy.deepcopy(Db.transaction_map[action_name])
    #         return None
    #     else:
    #         # 返回一个事务对象
    #         if Db.transaction_map.has_key(action_name):
    ...
    def toBson(obj):
        ...
        if '_id' in obj:
            ...
            if isinstance(obj['_id'], ObjectId):
                ...
                obj['_id'] = ObjectId(obj['_id'])
            elif isinstance(obj['_id'], bytes):
                obj['_id'] = ObjectId(obj['_id'].decode())
            ##id支持in条件查询
            elif '$in' in obj['_id']:
                inobj = [];
        ...
        if "updateAt" in obj and isinstance(obj['updateAt'],dict):
            item = obj['updateAt']
        ...
        if "createAt" in obj and isinstance(obj['createAt'],dict):
            item = obj['createAt']
            for key, value in item.items():
                if isinstance(value, str) or isinstance(value, unicode):
                    try:
